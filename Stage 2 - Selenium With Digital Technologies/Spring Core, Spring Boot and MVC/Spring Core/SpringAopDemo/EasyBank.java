...
public class EasyBank {
    ...
    public void doChangePin(int oldPin, int pin) {
        ...
        if (oldPin != pinCode || !Integer.toString(pin).matches("^[1-9]{1}[0-9]{3}$")) {
            // Actually an exception must be thrown here. But as we can't change the
            // signature
            // of this method (due to SkeletonValidator) we have to stick to the below print
            // and
            // return statement to run this code successfully. It's actually INCORRECT as we
            // will
            // be getting the output "You have successfully changed your pin" (due to
